...
/* ignoreManual=true מחזיר את החישוב הנקי, כדי שנוכל להציג
   אותו כרמז ליד השדות הידניים. */
function compute(P, ignoreManual){
  P = P || state.profile;
  ...
  const out = {bmr:Math.round(bmr), tdee:Math.round(tdee), target:Math.round(target), clipped, floor,
               protein, carbs, fat, bmi, water:round(w*0.035,1), manual:false};

  /* יעדים ידניים גוברים על החישוב. הם חלק מהפרופיל, ולכן
...
['age','height','weight','activity','goal'].forEach(id => {
  $(id).addEventListener('input', () => {
    ...
    renderProfile(); renderSummary(); renderTargets(); saveProfile();
  });
});
...
function renderTargets(){
  ...
  [...$('tgtMode').children].forEach(b =>
    b.setAttribute('aria-pressed', String((b.dataset.v === 'manual') === !!t.on)));
  $('tgtFields').hidden = !t.on;
